...
<Accordions>
  <Accordion title="What is cmdocs?">
    cmdocs is a self-hosted documentation platform that builds beautiful static sites from MDX.
  </Accordion>
  <Accordion title="Is it free?">
    Yes — cmdocs is open-source and free to use.
  </Accordion>
  <Accordion title="Can I self-host?">
    Absolutely. The build output is fully static — host it on any CDN or web server.
  </Accordion>
</Accordions>
...
Display a site-wide announcement at the top of every page (configured globally, not per page):

```mdx
<Banner>
  cmdocs v1.0 is out — read the release notes!
</Banner>
```
